...
func FindWeekendGetaways(ctx context.Context, origin string, opts WeekendOptions) (*WeekendResult, error) {
	...
	if len(exploreResult.Destinations) > 0 {
		apiCurrency = flights.DetectSourceCurrencyWithClient(ctx, client, origin, exploreResult.Destinations[0].AirportCode)
	}
	...
	for i, d := range dests {
		...
		go func(idx int, dest models.ExploreDestination) {
			...
			hr, searchErr := hotels.SearchHotelsWithClient(ctx, client, cityName, hotelOpts)
