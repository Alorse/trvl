...
var database = map[string]AirlineBaggage{
	...
	"LH": {
		...
		PersonalItem:      true,
		CheckedIncluded:   0,
		CheckedSource:     "https://www.lufthansa.com baggage calculator run for FRA-LIM: \"Free Baggage — Included in: Economy Light — 1 x Carry-on baggage, 1 x Personal item\". No checked piece; a first bag is quoted under Optional baggage",
		CheckedVerified:   "2026-08",
		CheckedFeeMin:     75,
		CheckedFeeMax:     120,
		FeeCurrency:       "USD",
		FeeSource:         "Same calculator, same route, prices shown in USD: first bag USD 75 online, USD 110 at the check-in counter, both \"Minimum fare per passenger\", plus the page's stated surcharge of up to 10 USD per bag depending on booking moment and departure month — hence a ceiling of 120",
		FeeVerified:       "2026-08",
		Notes:             "Corrects an entry that asserted 1x23kg on the strength of an inference — its own citation ended \"INFERRED: no per-brand row published\" — and claimed the zero-bag Light brand was \"confined to Scandinavia-US routes\". The calculator disproves that on FRA-LIM, which is neither. This was the costly direction: unlike the uncited claims that merely expire into unknown, this one was dated and cited, so Lufthansa PASSED a bag filter and reported its all-in as the bare fare. The earlier fee range of EUR 15-100 came from a 2023 group PDF spanning short-haul, and its floor understated a long-haul first bag. As with Austrian, the calculator also lists an Economy Basic fare below Light. Lufthansa does give a free first bag on US-Germany Economy, which is a different route and a different brand from the one a price search surfaces.",
	},
	"BA": {
		Code:              "BA",
	...
	"OS": {
		...
		PersonalItem:      true,
		CheckedIncluded:   0,
		CheckedSource:     "https://www.austrian.com/at/en/baggage-calculator run for FRA-LIM: \"Included in: Economy Light — Standard baggage: 1 x Carry-on baggage, 1 x Personal item\". No checked piece appears; a first bag is quoted under Additional Baggage instead",
		CheckedVerified:   "2026-08",
		CheckedFeeMin:     65,
		CheckedFeeMax:     110,
		FeeCurrency:       "EUR",
		FeeSource:         "Same calculator, same route: first bag EUR 65 booked online, EUR 100 at the check-in counter, both labelled \"Minimum fare per passenger\", plus the page's own caveat that \"an additional amount of up to 10EUR ... may be charged for each bag, depending on the moment of booking and month of departure\" — hence a ceiling of 110 rather than 100",
		FeeVerified:       "2026-08",
		Notes:             "Corrects an entry that claimed \"1x23kg checked bag included on most fares\" with no source behind it. It was not merely uncited, it was backwards: Austrian's cheapest intercontinental brand carries none, which is the same shape that proved wrong for Iberia, KLM, British Airways and SWISS. The staleness guard had already demoted it to unknown for want of a date, so it was not asserting the false claim — but it was costing 13 results on a single sweep. The calculator also notes an Economy Basic fare below Light; Basic cannot carry less than Light's zero, so the verdict holds for whichever the search surfaces. A second bag is EUR 200 online and EUR 260 at the counter.",
	},
	"LO": {
		Code:              "LO",
